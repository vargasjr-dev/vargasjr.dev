/**
 * Copies @vellumai/web/dist → public/assistant/
 * Run as part of the build: "prebuild": "bun scripts/copy-assistant.ts"
 *
 * The SPA's index.html hardcodes /assistant/ as its base path, so the
 * contents must live at public/assistant/ for Next.js static serving.
 *
 * After copying, applies patches to the SPA bundle so self-hosted
 * (docker/cloud) mode works without a gateway port.
 */
import { cp, mkdir, readFile, rm, writeFile } from "fs/promises";
import { join } from "path";

const srcDir = join(process.cwd(), "node_modules/@vellumai/web/dist");
const destDir = join(process.cwd(), "public/assistant");

await rm(destDir, { recursive: true, force: true });
await mkdir(destDir, { recursive: true });
await cp(srcDir, destDir, { recursive: true });
console.log("✅ Copied @vellumai/web/dist → public/assistant/");

// Patch the SPA bundle for self-hosted mode.
// Vercel uses a custom buildCommand (vercel.json) that bypasses package.json
// build scripts, so patches must be applied here after copying.
const patches: Array<{
  file: string;
  description: string;
  from: string;
  to: string;
}> = [
  {
    file: "assets/local-mode-DTyhlxIJ.js",
    description:
      "K() falls back to lockfile url+token when no gatewayPort, so YX() interceptor gets a token to inject as Authorization header",
    from: "async function K(){let e=v();if(!e)return;",
    to: "async function K(){let e=v();if(!e){let lf=j();lf.url&&lf.token&&x({url:lf.url,token:lf.token});return}",
  },
  {
    file: "assets/auth-store-B6yNvrw4.js",
    description:
      "In self-hosted docker branch (m()&&!v()), call g() to set b.url/b.token from lockfile before any API requests; remove allauth fire-and-forget that always 401s",
    from: "if(m()&&!v()){e({isLoggedIn:!0,isLoading:!1,user:Y}),a().then(t=>{t.ok&&t.data.user&&e({hasPlatformSession:!0,user:K(t.data.user)})}).catch(()=>{});return}",
    to: "if(m()&&!v()){try{await g()}catch{}e({isLoggedIn:!0,isLoading:!1,user:Y});return}",
  },
];

for (const { file, description, from, to } of patches) {
  const filePath = join(destDir, file);
  const content = await readFile(filePath, "utf-8");

  if (content.includes(to)) {
    console.log(`⏭️  Already patched: ${file}`);
    continue;
  }

  if (!content.includes(from)) {
    console.warn(`⚠️  Patch target not found in ${file} — skipping`);
    console.warn(`    ${description}`);
    continue;
  }

  await writeFile(filePath, content.replace(from, to));
  console.log(`🩹 Patched: ${file}`);
  console.log(`    ${description}`);
}
